...
while optimizeIndex <= 2:
    ...
    for train, test in kfold.split(input, output):
        ...
        model.add(Dense(parameters[0], input_dim=39,  kernel_regularizer=regularizers.l2(parameters[1]), activation='relu'))
        model.add(Dense(1, activation='sigmoid'))
        #compile model
        model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
        #fit model
        model.fit(input[train], output[train], epochs=parameters[2], batch_size=32, verbose=0)
        #evaluate model
